		...
		try{
			...
			for(var x in gpuNamesAll){
				...
				if(enabled?.match('1')){
					...
					if(deviceID != ''){
						name = gpuIDTable[deviceID]
					}
					
					var device = {}
	
					device.core = {}
					device.core.usage = parseFloat(gpu["utilization.gpu"])
					
					device.name = name
					
					device.memory = {}
					//device.memory.usage = parseFloat(gpu?.utilization?.memory_util)
					device.memory.bytes = parseUnit(gpu["memory.used"])
					device.memory.bytes_total = parseUnit(gpu["memory.total"])
					...
					device.power = {}
					device.power.watts = parseFloat(gpu["power.draw"])
					device.power.watts_limit = parseFloat(gpu["power.limit"])
					...
					device.bus.generation_max = parseInt(gpu["pcie.link.gen.max"])
					device.bus.width_max = parseInt(gpu["pcie.link.width.max"])
					device.bus.generation = parseInt(gpu["pcie.link.gen.current"]) || device.bus.generation_max
					device.bus.width = parseInt(gpu["pcie.link.width.current"]) || device.bus.width_max
					
					if(!device.bus.generation && device.bus.tx_bytes == undefined){
						delete device.bus
					}
					
					device.fan_speed = parseFloat(gpu["fan.speed"])
					
					device.temperature = parseFloat(gpu["temperature.gpu"])
